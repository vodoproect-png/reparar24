// @ts-nocheck
/**
 * Creates or finds the Google Ads WhatsApp click conversion action.
...
const CONVERSION_NAME = 'WhatsApp click - Reparar24'
...
async function main() {
  ...
  if (!conversion) throw new Error('Could not read WhatsApp conversion action after create.')
  ...
  const output = path.join(OUT_DIR, 'whatsapp-conversion-action.json')
  fs.writeFileSync(output, `${JSON.stringify(report, null, 2)}\n`, 'utf8')

  console.log(`WhatsApp conversion action ${created ? 'created' : 'already exists'}.`)
